...
class VendorCatalogConfig(models.Model):
    ...
    def action_run_import(self):
        ...
        if self.feed_format in ("json","csv") and not self.feed_url:
            raise UserError(_t("Debes indicar la URL del feed."))
        if self.feed_format == "python" and not (self.python_module and self.python_callable):
            raise UserError(_t("Indica módulo y función Python."))
        ...
        for idx, raw in enumerate(items, start=1):
            try:
                tmpl, was_created = self._upsert_product(raw)
                created += 1 if was_created else 0
                updated += 0 if was_created else 1
            except Exception as e:
                skipped += 1
                code = raw.get("sku") or raw.get("default_code") or raw.get("barcode") or "N/A"
                details.append(f"ERROR item {code}: {e}")
            if self.batch_commit and (idx % self.batch_commit == 0):
                self.env.cr.commit()

        msg = f"Total: {len(items)} | Creados: {created} | Actualizados: {updated} | Fallidos: {skipped}"
        now = fields.Datetime.now()
        self.sudo().write({'last_run': now, 'last_result': msg + ("\n" + "\n".join(details[:200]) if details else "")})
        self.env["vendor.catalog.log"].create({
            "config_id": self.id, "run_date": now,
            "total": len(items), "created": created, "updated": updated, "skipped": skipped,
            "notes": msg,
        })
        return {"type":"ir.actions.client","tag":"display_notification","params":{"title":_t("Catálogo importado"),
                "message":f"{msg}\n{_t('Última ejecución')}: {now}","type":"success" if skipped==0 else "warning","sticky":False}}
